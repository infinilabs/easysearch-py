...
of ``easysearch`` repository existing on the same level as the
``easysearch-py`` clone. Before running the tests we will, by default, pull
 ...
 * ``EASYSEARCH_HOST`` - can contain "hostname[:port]" of running es cluster

 * ``PYTHON_CLASS_CONNECTION`` - name of the connection class to use from
    ``easysearch.connection`` module. If you want to run completely with your
    own see section on customizing tests.

 * ``TEST_ES_YAML_DIR`` - path to the yaml test suite contained in the
    easysearch repo. Defaults to ``$TEST_ES_REPO/rest-api-spec/test``

 * ``TEST_ES_REPO`` - path to the easysearch repo, by default it will look in
    the same directory as ``easysearch-py`` is in. It will not be used if
    ``TEST_ES_YAML_DIR`` is specified directly.

 * ``TEST_ES_NOFETCH`` - controls if we should fetch new updates to easysearch
   repo and reset it's version to the sha used to build the current es server.
   Defaults to ``False`` which means we will fetch the easysearch repo and
 ...
 * to run a specific test, you can use ``python3 setup.py test -s <test_name>``, for example
   ``python3 setup.py test -s test_easysearch.test_helpers.TestParallelBulk.test_all_chunks_sent``

To install all test dependencies you can also run ``pip install -e .[develop]``.

Run Easysearch in a Container
--------------------------------

To run easysearch in a container  set the ``EASYSEARCH_VERSION``
environment variable to 7.5, 7.5-SNAPSHOT. Then run ./.ci/run_easysearch.sh::

    # Run OSS Easysearch v7.5
    export EASYSEARCH_VERSION=easysearch-oss:7.5-SNAPSHOT
    ./.ci/run_easysearch.sh

    # Run XPACK Easysearch v7.5
    export EASYSEARCH_VERSION=easysearch:7.5-SNAPSHOT
    ./.ci/run_easysearch.sh

This will run a version for Easysearch in a Docker container suitable for
running the tests. To check that easysearch is running first wait for a
...
You can create a `local.py` file in the `test_easysearch` directory which
should contain a `get_client` function.

If this file exists the function will be used instead of
`easysearch.helpers.test.get_test_client` to construct the client used for
any integration tests. You can use this to make sure your plugins and
extensions work with `easysearch-py`.
